...
function MyApp() {
    ...
    async function makePatchCall(person, index) {
        try {
            const response = await axios.patch('http://localhost:5005/tasks/'.concat(characters[index]['_id']), person);
            return response;
        }
         catch (error) {
            console.log(error);
    ...
    return (
        <div className="container">
          ...
          <Table characterData={characters} removeCharacter={removeOneCharacter} handleEdit = {editList}/>
        </div>
      );
